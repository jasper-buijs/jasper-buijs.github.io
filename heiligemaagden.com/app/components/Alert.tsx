interface AlertProps {
  type: "succes" | "warning" | "danger" | "info";
  hideIcon?: boolean;
  text: string;
}
const Alert = ({ type, hideIcon, text }: AlertProps) => {
  ...
  const broderColor = {
    succes: "border-[#a3cfbb]",
    warning: "border-[#ffe69c]",
    danger: "border-[#f1aeb5]",
    info: "border-[#9ec5fe]"
  }
  const icons = {
    succes: <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="1rem" height="1rem" className="inline fill-[#0a3622] mr-1"><path d="M1 12C1 5.925 5.925 1 12 1s11 4.925 11 11-4.925 11-11 11S1 18.075 1 12Zm16.28-2.72a.751.751 0 0 0-.018-1.042.751.751 0 0 0-1.042-.018l-5.97 5.97-2.47-2.47a.751.751 0 0 0-1.042.018.751.751 0 0 0-.018 1.042l3 3a.75.75 0 0 0 1.06 0Z"></path></svg>,
  ...
  return (
    <div className={`my-4 p-4 l-0 r-0 h-fit relative ${backgroundColors[type]} ${textColors[type]} border-l-4 ${broderColor[type]} border-solid`}>
      {!hideIcon && icons[type]}
      {text}
    </div>
  );
