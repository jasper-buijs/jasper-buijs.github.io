...
const Alert = ({ type, hideIcon, text }: AlertProps) => {
  ...
  const buttonBorderColors = {
    success: "border-[#0a3622]",
    warning: "border-[#664d03]",
    danger: "border-[#58151c]",
    info: "border-[#052c65]"
  }
  const broderColor = {
  ...
  const icons = {
    ...
    success: <CircleCheckBig className={"inline-block align-middle mr-1"} />,
    ...
    warning: <TriangleAlert className={"inline-block align-middle mr-1"} />,
    ...
    danger: <OctagonX className={"inline-block align-middle mr-1"} />,
    ...
    info: <Info className={"inline-block align-middle mr-1"} />
  }
  let [close, setClose] = useState<boolean>(false);
  const closeAlert = () => setClose(true);
  return (
    <div className={`p-4 mt-1 l-0 r-0 h-fit relative ${backgroundColors[type]} ${textColors[type]} border-l-4 ${broderColor[type]} border-solid ${close && "!hidden"}`}>
      {!hideIcon && icons[type]}
      <b className={"inline-block align-middle"}>{(type == "danger" || type == "warning") ? "IMPORTANT" : "INFO"}</b><br />
      {text}
      <p className="block w-full text-center mt-2">
        <button onClick={closeAlert} className={`border ${buttonBorderColors[type]} px-2 rounded-md`}>Close</button>
      </p>
    </div>
  );
}
