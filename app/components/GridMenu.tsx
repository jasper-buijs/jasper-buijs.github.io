...
const GridMenuElement = ({ bgImage, title, href, columns, rows }: GridMenuElementProps) => {
  ...
  return (
    <a href={href} className={`relative group ${gridClasses()}`}>
      <div style={divStyle} className={`absolute w-full h-full ${bgClasses()} group-hover:brightness-[30%] transition-all !duration-[600]`}>
      </div>
      <div className={`absolute w-full brightness-100 ${textClasses()} text-4xl text-center top-[75%] group-hover:underline transition-all !duration-[600]`}>
        {title}
      </div>
    </a>
  );
